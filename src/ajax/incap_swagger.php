...
if (substr($endpoint,0,4)=='http') {
	error_log($logDate." | Incapsula SWAGGER Request: curl ".$endpoint);
	...
	error_log($logDate." | Incapsula SWAGGER Response: ".$response);
	if(curl_exec($ch) === false){
		var_dump(curl_error($ch));
	}	
	curl_close($ch);
} else {
	$response = file_get_contents('../assets/cwaf/'.$_GET["endpoint"]); 
}

$parsed = (isJson($response) ? json_decode($response) : (object) yaml_parse($response));
$parsed->apiDefinitionIndex = $_GET["apiDefinitionIndex"];
